...
def svm_model (svm_C=100,svm_kernel='rbf', num_output = 1):
    """ perform a SVM
    Args:
...
def dtc_model(rf_max_depth=-1,rf_min_samples_split=2 , rf_random_state = None, rf_max_features=-2):
    ...
    if rf_max_depth == -1:
        return DecisionTreeClassifier(min_samples_split=rf_min_samples_split, max_features=rf_max_features, random_state=rf_random_state)
    else:
        return DecisionTreeClassifier(max_depth=rf_max_depth, min_samples_split=rf_min_samples_split, max_features=rf_max_features, random_state=rf_random_state)

# def rf_model (rf_n_estimators=500,rf_max_depth=-1,rf_min_samples_split=2 , rf_random_state = None, rf_max_features=-2):
def rf_model (rf_n_estimators=100,rf_max_depth=30,rf_min_samples_split=2 , rf_random_state = None, rf_max_features=-3):
    """ Random Forest
    Args:
    ...
    if rf_max_features ==-2:
        ...
        rf_max_features_v = None
    elif rf_max_features ==-3:
        rf_max_features_v = 'sqrt'
    elif rf_max_features ==-4:
        rf_max_features_v = 'log2'
    else:
        rf_max_features_v = rf_max_features
    if rf_max_depth == -1:
...
def model_cnn4_dropout(input_reshape,num_classes, optimizers_func='SGD') :
    # optimizers_func=optimizers.SGD(lr=0.0005, momentum=0.1, decay=1e-6)
    model = Sequential()
    model.add(InputLayer(input_shape=input_reshape))  
...
def model_cnn1d(input_reshape,num_classes=112, optimizers_func ='adam', 
        numfilter=32, filtersize=3,numlayercnn_per_maxpool=1,nummaxpool=1,maxpoolsize=2,
        dropout_cnn=0.2, dropout_fc=0.4,padded='True',lr_rate=0.001,lr_decay=1e-05, loss_func='sparse_categorical_crossentropy' ):   
...
def call_model (type_model,m_input_shape, m_num_classes, m_optimizer,m_learning_rate, m_learning_decay,m_loss_func,
            ...
            svm_c, svm_kernel, rf_n_estimators,rf_max_depth, m_pretrained_file,knn_n_neighbors, rf_max_features,m_type_emb=''):
    ...
    if type_model== "fc_model":
        ...
        model=model_cnn(m_input_shape,m_num_classes,
            ...
            dropout_cnn=mc_dropout_rate_cnn, dropout_fc = ml_dropout_rate_fc, padded = mc_padding, type_emb=m_type_emb) 
