...
def build_breaks (data,type_bin,num_bin=10,max_v=1,min_v=0):
    ...
    if type_bin == 'eqf':
        ...
        for i in range (0,num_bin-1):
            #print("int(count_value/num_bin)*i=")
            #print(int(count_value/num_bin)*i)
            bins_break = np.append(bins_break,(data_f_excluded_sorted [int(count_value/num_bin)*i]))
    elif type_bin == 'eqw':
    ...
    ######## end of Author: nhi ######### 
        ...
        gmm = GaussianMixture(n_components=num_bin - 1).fit(data_f_excluded)
        labels = gmm.predict(data_f_excluded)

        # step 4: Calculator bins_break
        bins_break = get_bin_break(bins_break, data_f_excluded, labels)
         ######## Author: xuan dai #########    
    ...
    return bins_break
        ...
        ######## Author: xuan dai #########    
def build_breaks_gmm (data,type_bin,num_bin=10,max_v=1,min_v=0,
    ...
    #  tol=0.001, reg_covar=1e-06, max_iter=100, n_init=1,
    #   init_params='kmeans', # init_params{‘kmeans’, ‘k-means++’, ‘random’, ‘random_from_data’}, default=’kmeans’
    #  weights_init=None, 
    #  means_init=None, 
...
def embed_image (X_embedded, X, color_arr,colormap, file_name_emb, 
    ...
    # begin: nabang@
    if fig_size == -2:  #fig_size scales depending on the x,y axises for decrete values.
        
        lech_x = abs(np.min(X_embedded[:,0]) - np.max(X_embedded[:,0]))
        lech_y = abs(np.min(X_embedded[:,1]) - np.max(X_embedded[:,1]))
        
        fig_size = max(lech_x,lech_y)
    # end: nabang@   
...
def generate_QRcode(data, file_name_emb, len_string=2678, size=512, data_type_qrcode='numeric'):
    '''
    description: generate image with QR code
    arg:
        sample: a sample
        file_name_emb: filename
        size=2678 (max)
        data_type: numeric or text

    return:
        an image with filename
    '''
    import qrcode

    if data_type_qrcode == 'numeric':
        my_string = ' '.join(str(data))
        #data1 = my_string[1:len_string]
    else: # string, text
        my_string = str(data)
