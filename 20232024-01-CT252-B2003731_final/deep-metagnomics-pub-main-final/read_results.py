...
print ("the results are collecting... ")
...
parser.add_option("-d", "--debug", type="string", default="n", help="show output y or n (default: n)") 
...
title_cols = np.array([[ "filename","time",	"train_acc"	,"val_acc",	"sd_acc10",	"val_auc"	,"sd_auc10",	"val_tn",	"val_fn",	"val_fp",	"val_tp",	"val_precision"	,"sd_precision10",	"val_recall",	"sd_recall10",	"val_f1",	"sd_f1_10",	"val_mmc",	"sd_mcc_10",	"epst", "train_acc_a",	"sd_acc",	"val_acc_a",	"sd_acc"	,"train_auc_a",	"sd_auc"	,"val_auc_a",	"sd_auc"	,"va_mmc_a",	"sd_mmc"]])
np.savetxt(f,title_cols, fmt="%s",delimiter="\t")
...
if options.mode_read >= 3:
    ...
    print ("The results of " +str(num_files) + " experiments ("+str(num_files2+num_files)+ " results) are collected!")
elif options.mode_read >= 2:
    if options.debug == 'y':
        print (str(num_files) + ' file(s) was/were summarized to ' + str(options.folder_output_results) + str(file_sum))
        print (str(num_files) + ' file was/were summarized to ' + str(options.folder_output_results) + str(file_folds))
    print ("The results of " +str(num_files) + " experiments  are collected!")
else:
    if options.debug == 'y':
        print (str(num_files) + 'file(s) was/were summarized to ' + str(options.folder_output_results) + str(file_sum))
    print ("The results of " +str(num_files) + " experiments  are collected!")
