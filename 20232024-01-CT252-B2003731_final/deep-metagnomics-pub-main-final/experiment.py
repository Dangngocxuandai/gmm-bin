...
try:
    ...
    print('error when importing vis_data')
    print('importing from deepmg')
    from deepmg import vis_data

try:
    import utils_deepmg as utils
except ImportError:
    print('error when importing utils_deepmg')
    print('importing from deepmg')
...
from sklearn.metrics import roc_auc_score,accuracy_score,precision_score,recall_score,matthews_corrcoef, confusion_matrix, log_loss#, multilabel_confusion_matrix
from sklearn.metrics import f1_score as f1_scores_computation
...
# from kr.src.utils.np_utils import to_categorical
...
def para_cmd ():
    ...
    #setting for experiment: divide data/time run
    ...
    parser.add_option('-k','--n_folds',help='number of k folds (default:2), if k == 1, training on whole *_x.csv, and testing on *z_x.csv')
    ...
    parser.add_option('--save_rf',help='save important features and scores for Random Forests [y/n]')
    ...
    parser.add_option('--transpose_data',help='transpose data [y/n]? (default:y)')

    #binning approaches/dimensionality reduction/preprocessing and scaling
    ...
    parser.add_option('-v','--visualize_model',help='show the structure of model if > 0 (default:0)')
    parser.add_option('--algo_redu',help='algorithm of dimension reduction (rd_pro/pca/fa),  if          emtpy so do not use (default:'')')
    parser.add_option('--reduc_perle',help='perlexity for tsne using in dimensionality reduction (default:10)')
    ...
    parser.add_option('-y','--type_bin',help='type of binnings: spb/eqw/pr/eqf/kmeans (default:'')')
    ...
    parser.add_option('--gmm_init_params',help='please select in [kmeans, k-means++, random, random_from_data]')
    parser.add_option('--gmm_covariance_type',help='please select in [full, tied, diag, spherical], default=full')
    ...
    parser.add_option('--iter_visual',help='#iteration for run algorithm for visualization; for tsne, it should be at least 250, but  do not set so high (default:300); for minisom (should 100)')
    ...
    parser.add_option('--fig_size',help='fig_size to contain all features, if 0:smallest which fit data, -1 use real images, -2: scaling the size depending on x,y axises applying to decreted coornidates (default:0)')
    ...
    parser.add_option('--vis_nclusters',help=' the number of clusters for clustering in visualiztion')
    parser.add_option('--bs_vis_kmeans',help=' batch size in mini batch size k-mean clustering')
    parser.add_option('--len_stringqrcode',help='length for QR code generator')
    parser.add_option('--data_type_qrcode',help='data type for QR code generator')
    

    #grid_search, use for selecting parameters, now only support for training
    ...
    parser.add_option('-e','--epoch',help='the epoch used for training (default:5)')
...
def get_default_value (options):
    ...
    if not options.transpose_data:  #if did not set from cmd, use default value
            options.transpose_data = 'y'    
    ...
    if not options.vis_nclusters:  #if did not set from cmd, use default value
        options.vis_nclusters = '10'

    if not options.len_stringqrcode:  #if did not set from cmd, use default value
        options.len_stringqrcode = '100'

    if not options.data_type_qrcode:  #if did not set from cmd, use default value
        options.data_type_qrcode = 'numeric'
    # data_type_qrcode

    # if not options.data_type_qrcode:  #if did not set from cmd, use default value
    #     options.gmm_init_params = 'kmeans'
    # if not options.data_type_qrcode:  #if did not set from cmd, use default value
    #     options.data_type_qrcode = 'numeric'
    return options
...
def para_config_file (options):
    ...
    if not options.path_data_vis_learn:  #if did not set from cmd
        if config.has_option('Vis_learn','path_data_vis_learn'): #if avaiable from config file
            ...
            options.path_data_vis_learn = ''
    if not options.transpose_data:  #if did not set from cmd
        if config.has_option('Vis_learn','transpose_data'): #if avaiable from config file
            options.transpose_data = config.get('Vis_learn','transpose_data')
        else: #other cases, use default value
            options.transpose_data = 'y'
    ...
    if not options.recreate_img:  #if did not set from cmd
        if config.has_option('Visual','recreate_img'): #if avaiable from config file
            ...
            options.recreate_img = '0'
    if not options.vis_nclusters:  #if did not set from cmd
        if config.has_option('Visual','vis_nclusters'): #if avaiable from config file
            options.vis_nclusters = config.get('Visual','vis_nclusters')
        else: #other cases, use default value
            options.vis_nclusters = '10'

    if not options.len_stringqrcode:  #if did not set from cmd
        if config.has_option('Visual','len_stringqrcode'): #if avaiable from config file
            options.len_stringqrcode = config.get('Visual','len_stringqrcode')
        else: #other cases, use default value
            options.len_stringqrcode = '100'
    
    if not options.data_type_qrcode:  #if did not set from cmd
        if config.has_option('Visual','data_type_qrcode'): #if avaiable from config file
            options.data_type_qrcode = config.get('Visual','data_type_qrcode')
        else: #other cases, use default value
            options.data_type_qrcode = 'numeric'
...
def string_to_numeric(options):
    ...
    options.len_stringqrcode = int(options.len_stringqrcode)
    ...
    options.vis_nclusters =  int(options.vis_nclusters)  
...
def write_para(options,configfile):
    ...
    Config.set('io','transpose_data',options.transpose_data)
    ...
    Config.set('Visual','vis_nclusters',str( options.vis_nclusters ))
    Config.set('Visual','len_stringqrcode',options.len_stringqrcode )
    Config.set('Visual','data_type_qrcode',options.data_type_qrcode )
...
def validation_para(options):
    ...
    if options.data_name == '' and options.parent_folder_img == 'images': # modified by tbtoan
    ...
    if not (options.type_emb in ['raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom','none','qrcode']):
        print  ('you entered --type_emb=' + str(options.type_emb))
        print  (utils.textcolor_display('--type_emb') + " only supports 'raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom, 'qrcode")
        valid = 0
    ...
    if not (options.type_bin in ['spb','pr','eqw','eqf','kmeans','kmedoids','','GMM']):
        print  ('you entered --type_bin=' + str(options.type_bin))
        print  (utils.textcolor_display('--type_bin') + " only supports 'spb','pr','eqw','eqf','GMM' or skip this option")
        valid = 0
    ...
    if options.transpose_data not in ['y','n','no','yes']:        
        print  ('you entered --transpose_data=' + str(options.transpose_data))
        print  (utils.textcolor_display('--transpose_data only accepts y/n/no/yes'))
        valid = 0
    
    if options.data_type_qrcode in ['text'] and options.type_bin != '':        
        print  ('you entered --data_type_qrcode=' + str(options.data_type_qrcode))
        print  (utils.textcolor_display('--data_type_qrcode=text only accepts type_bin is null, currently its value= '  + str(options.type_bin)))
        valid = 0
    if valid == 0:
        exit()
...
def naming_folder (options, search_res = 'y'):
    ...
    if not(options.orderf_fill in ["none"]): #set name for features ordered randomly
        if options.type_emb in ['raw','fill','fills','zfill','zfills']:
            ...
            elif options.orderf_fill in ['kmean','minibskmean']:
                pre_name = pre_name + options.orderf_fill + '_n' + str(options.vis_nclusters)
            else:
    ...
    if options.type_emb == 'raw':      #raw data  
        ...
        if not options.num_bin == '_':
            pre_name = pre_name + '_inipara_' + str(options.gmm_init_params) +  '_covariance_typ_' + str(options.gmm_covariance_type)
        ...
        if options.type_emb == 'qrcode':
            pre_name = pre_name + 'len' + str(options.len_stringqrcode)
...
def read_data (options, suffix = '', included_labels = 'y' , get_names_of_features = 'n', mode='internal') :
    ...
    ** orderf_fill used if and only if type_emb == 'fill' or a type of fill
    ...
    try:
        ...
        #with open(path_v_set_x, 'rb') as f:
        #    v_data_ori = f.read()
        v_data_ori = pd.read_csv(path_v_set_x,encoding='latin1')


    v_data_ori = v_data_ori.set_index('Unnamed: 0')
    
    if options.transpose_data in ['no','n']:
        v_data_ori = np.transpose(v_data_ori) 
    ...
    if options.orderf_fill != "none": #set name for features ordered randomly
        if options.type_emb in ['raw','fill','fills','zfill','zfills']:
            if options.orderf_fill not in  ['high','kmean','minibskmean']:             # other rankings
...
def create_fillup_folder (options,path_write_parentfolder_img,
    ...
    if not os.path.exists(path_write_parentfolder_img+'/'+prefix_img + '_' + str(len(labels)-1)+'.png') or options.recreate_img > 0 :
        ...
        # generate bin breaks
        if options.type_bin == 'GMM':
            bins_breaks = vis_data.build_breaks_gmm (data=data, type_bin=options.type_bin, num_bin=options.num_bin, 
                max_v= max_v_temp, min_v = min_v_temp,
                gmm_init_params = options.gmm_init_params, gmm_covariance_type =  options.gmm_covariance_type )
                #gmm_init_params = 'kmeans', gmm_covariance_type = 'full')
        else:
    ...
    if only_visual in ['n']:
        data = utils.load_img_util(num_sample = len(data), pattern_img= prefix_img,
                ...
                channel = options.channel,mode_pre_img = options.mode_pre_img,type_emb=options.type_emb)       
...
def visual_section (options,  train_x,val_x, train_y, val_y, v_data, v_labels,train_x_original,
    ...
    if options.type_emb != 'none': # added by tbtoan   
        ...
        if options.fig_size == 0: #if fig_size <=0, use the smallest size which fit the data, generating images of (len_square x len_square)
            if options.data_type_qrcode == 'text': # neu la chuoi cho kich thuoc anh 512x512
                fig_v_size = 512
            else:
        ...
        if options.data_type_qrcode!='text' : #nthai, neu text bo qua binning
        ...
        if options.type_emb in ['fills','zfills'] and options.type_bin != 'pr': #fills: use fill-up with bins created from train-set with scale, should be used with type_bin='eqw'
            ...
            if (not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png")) or options.recreate_img > 0 :
                ...
                # begin tbtoan
                elif options.orderf_fill == 'kmean':
                    
                    from sklearn.cluster import KMeans          
                    
                    kmeans = KMeans(n_clusters=options.vis_nclusters, random_state=0).fit(np.transpose(train_x))

                    labels_clusters_features = kmeans.labels_
                    print('---cluster kmean' + str(len(labels_clusters_features)))
                    print(labels_clusters_features)             
                    #index_ordered = sorted(list(zip(labels_clusters_features, mean_spe)), key=lambda x: x[0]) 
                    #index_ordered = sorted(range(len(labels_clusters_features)), key=lambda k: labels_clusters_features[k])
                    index_combine = list()
                    for i in range(0,len(labels_clusters_features)):
                        #print(str(n[i])+'_'+str(i))
                        index_combine.append((str(labels_clusters_features[i])+'_'+str(i)))
                    print(index_combine)
                    
                    #exit()

                    vis_data.generate_image (data = [x for _,x in sorted(zip(index_combine,mean_spe))], bins_breaks = bins_breaks, colormap = options.colormap, 
                        file_name_emb = path_write + "/global",size_p = options.point_size, fig_size = fig_v_size,
                        marker = options.shape_drawn, alpha_v = options.alpha_v,
                        cmap_vmin = options.cmap_vmin, cmap_vmax = options.cmap_vmax, margin= options.margin, 
                        min_v = min_v_temp, num_bin = options.num_bin,
                    ...
                    for i in range(0, len(train_y)):
                        print  ('img fillup-kmean train ' + str(i) )           

                        vis_data.generate_image (data = [x for _,x in sorted(zip(index_combine,train_x[i]))], bins_breaks = bins_breaks, colormap = options.colormap, 
                            file_name_emb = path_write + "/train_" + str(i),size_p = options.point_size, fig_size = fig_v_size,
                            marker = options.shape_drawn, alpha_v = options.alpha_v,
                            cmap_vmin = options.cmap_vmin, cmap_vmax = options.cmap_vmax, margin= options.margin, 
                            min_v = min_v_temp, num_bin = options.num_bin,
                    ...
                    for i in range(0, len(val_y)):
                        print  ('img fillup-kmean val ' + str(i))  
                        vis_data.generate_image (data = [x for _,x in sorted(zip(index_combine,val_x[i]))], bins_breaks = bins_breaks, colormap = options.colormap, 
                            file_name_emb = path_write + "/val_" + str(i),size_p = options.point_size, fig_size = fig_v_size,
                            marker = options.shape_drawn, alpha_v = options.alpha_v,
                            cmap_vmin = options.cmap_vmin, cmap_vmax = options.cmap_vmax, margin= options.margin, 
                            min_v = min_v_temp, num_bin = options.num_bin,
                ...
                # end tbtoan
                ...
                elif options.orderf_fill == 'minibskmean':
                    ...
                    kmeans = MiniBatchKMeans(n_clusters=options.vis_nclusters, random_state=0, batch_size=bs_vis_kmeans).fit(np.transpose(train_x))
                    
                    labels_clusters_features = kmeans.labels_
                    print('---cluster kmean' + str(len(labels_clusters_features)))
                    print(labels_clusters_features)    
                    #index_ordered = sorted(list(zip(labels_clusters_features, train_x)), key=lambda x: x[0])   
                    #index_ordered = sorted(range(len(labels_clusters_features)), key=lambda k: labels_clusters_features[k])   
                    #print('index_ordered==')  
                    #print(index_ordered)  
                    index_combine = list()
                    ...
                    for i in range(0, len(train_y)):
                        ...
                        vis_data.generate_image (data = [x for _,x in sorted(zip(index_combine,train_x[i]))], bins_breaks = bins_breaks, colormap = options.colormap, 
                            ...
                            cor_x=cordi_x,cor_y=cordi_y)   
                                ...
                                # kmeans = KMeans(n_clusters=n_cluster, random_state=0).fit(val_x)               
        ...
        elif options.type_emb in  ['tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom','qrcode'] : #if use other embeddings (not fill), then now embedding!
            ...
            if not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png") or options.recreate_img > 0 : 
                ...
                if options.type_emb =='tsne':
                ...
                elif options.type_emb =='gsom': 
                    path_r_coor = os.path.join(options.original_data_folder,  options.data_name, 
                            's'+ str(seed_value) + 'k'+str(index+1) +'_cor.csv') 
                    
                    X_embedded_r = pd.read_csv(path_r_coor)
                    #X_embedded = np.array (np.stack(X_embedded))
                    #X_embedded =  np.stack(X_embedded)
                    #print("X_embedded,gsom==")
                    #print(X_embedded.iloc[2,1] )
                    #read_labels.iloc[:,1] 
                    print('---coordinates')
                    print(X_embedded_r)
                    X_embedded = np.empty((0,2), int)
                    # print("len=")
                    # print(len(X_embedded_r))
                    # print('X_embedded_r[0]')
                    #print(X_embedded_r)
                    # #print(X_embedded_r[1]['V1'])
                    str_x_y = []
                    for i in range(len(X_embedded_r)) :
                    #     print("i,X_embedded_r[i]=")
                        #print('i1========',i)
                        #print(X_embedded_r.iloc[i,0])
                        #print('i2========',i)
                        #print(X_embedded_r.iloc[i,1])
                        X_embedded = np.append(X_embedded,[[X_embedded_r.iloc[i,0],X_embedded_r.iloc[i,1]]],axis=0)
                        str_x_y = np.append(str_x_y, str(X_embedded_r.iloc[i,0]) + '_' +str(X_embedded_r.iloc[i,1]))
                    print('X_embedded=========')
                    ...
                    # nthai: BEGIN: fix size=0 in minisom, version 1.0.37
                    if options.fig_size == 0: 
                        fig_v_size =  int(math.ceil(math.sqrt(input_x.shape[1])))    
                    else:
                        fig_v_size = options.fig_size
                    

                    som = MiniSom(fig_v_size,fig_v_size,input_x.shape[1],
                        sigma=options.perlexity_neighbor,learning_rate=options.lr_visual)   
                    # nthai: END: fix size=0 in minisom, version 1.0.37
                    ...
                    if options.type_emb in ['qrcode']:
                        print('you are running with QRcode######### QRcode does not use binning techniques')
                        #continue
                    else:
                ...
                print(input_x.shape)
                print(input_x)
                if options.type_emb != 'qrcode' and options.data_type_qrcode!='numeric':
                    mean_spe=(np.mean(input_x,axis=1))    
                if options.type_emb in ['qrcode']:
                    print('##########you are running with QRcode######### QRcode does not use binning techniques')
                    #import qrcode
                    for i in range(0, len(train_x)):       
                        vis_data.generate_QRcode(data = train_x [i,:], file_name_emb = path_write+"/train_"+str(i)+'.png', 
                                                 len_string=options.len_stringqrcode, data_type_qrcode = options.data_type_qrcode, size = fig_v_size)
                    for i in range(0, len(val_x)):   
                        vis_data.generate_QRcode(data = val_x [i,:], file_name_emb = path_write+"/val_"+str(i)+'.png' ,
                                                  len_string=options.len_stringqrcode , data_type_qrcode = options.data_type_qrcode, size = fig_v_size)
                else:
    ...
    train_x_img = utils.load_img_util(num_sample = len(train_y), pattern_img='train',
        path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
        channel = options.channel,mode_pre_img = options.mode_pre_img, type_emb=options.type_emb,data=train_x)
    val_x_img = utils.load_img_util(num_sample = len(val_y), pattern_img='val',
        path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
        channel = options.channel,mode_pre_img = options.mode_pre_img, type_emb=options.type_emb,data=val_x)
    if options.test_ext == 'y':
        v_data = utils.load_img_util(num_sample = len(v_labels), pattern_img='testval',
            path_write = path_write , dim_img = options.dim_img, preprocess_img = options.preprocess_img, 
            channel = options.channel,mode_pre_img = options.mode_pre_img,type_emb=options.type_emb,data=v_data)

    return train_x_img,val_x_img,v_data
...
def run_kfold_deepmg(options,args):
    ...
    if options.type_run not in ['vis','visual']:  
        ...
        try:
            ...
            print('error when importing models_def')
            print('importing from deepmg')
        ...
        from tensorflow.keras import backend as optimizers
        from tensorflow.keras.models import Sequential
        from tensorflow.keras.layers import Activation, Dropout, Flatten, Dense, InputLayer, Conv2D, MaxPooling2D,Input
    ...
    # ==============================================================================
    if options.type_emb == "gsom":
        path_r_set_data = os.path.join(options.original_data_folder,  options.data_name, 's1k1_train_x.csv')
        print(options.original_data_folder,  options.data_name, 's1k1_train_x.csv')
        #labels = [1]
        print("data=")
        data = pd.read_csv(path_r_set_data, header = 0)
        #print(data)
        ...
        path_r_set_y = os.path.join(options.original_data_folder,  options.data_name, 's1k1_train_y.csv') 
        labels = pd.read_csv(path_r_set_y , header = 0)
        labels = labels.iloc[:,1] 
        #name_features = [1]
    
    # added by tbtoan BEGIN ### ---------### ---------
    elif options.type_emb == "none":
        from pathlib import Path

        #data_dir = Path(os.getcwd() + '/' + options.parent_folder_img)
        data_dir =  options.parent_folder_img
        ...
        if os.path.isdir(data_dir):
            ...
            train_x, train_y = utils.read_from_directoty(train_directory)
            val_x, val_y = utils.read_from_directoty(test_directory)

            # combine train and test set
            data = np.asanyarray(train_x + val_x, dtype=object)
            labels = np.append(train_y, val_y)

        else:
            print('Folder {} is not exists'.format(data_dir))
            exit()
    # added by tbtoan END ### --------### ---------
    else:
    ...
    # modified by tbtoan  ---####### begin ####--------
    if options.type_emb != 'none': # modified by tbtoan
        if options.fig_size == 0: #if fig_size <=0, use the smallest size which fit the data, generating images of (len_square x len_square)
            fig_v_size =  int(math.ceil(math.sqrt(data.shape[1])))    
        else:
            fig_v_size = options.fig_size
    # modified by tbtoan  ---####### end ####--------
    ...
    for seed_value in range(options.seed_value_begin,options.seed_value_begin+options.run_time) :
        ...
        for index, (train_indices,val_indices) in enumerate(skf.split(data, labels)):
            ...
            if options.type_emb == 'gsom':
                ## read train data
                path_r_set_x = os.path.join(options.original_data_folder,  options.data_name, 's'+ str(seed_value) + 'k'+str(index+1) +'_train_' +  'x.csv') 
                print(path_r_set_x)
                train_x = pd.read_csv(path_r_set_x, header = 0) #rows: features, colulms: samples 
                #train_x = train_x.set_index('Unnamed: 0')
                #train_x = train_x.set_index()
                train_x = train_x.values
                train_x = np.transpose(train_x) #rows: samples, colulms: features
                train_x = np.stack(train_x)  
                

                path_r_set_y = os.path.join(options.original_data_folder,  options.data_name, 's'+ str(seed_value) + 'k'+str(index+1) +'_train_' +  'y.csv') 
                read_labels = pd.read_csv(path_r_set_y)
                train_y = read_labels.iloc[:,1] 

                ## read test data
                path_r_set_x = os.path.join(options.original_data_folder,  options.data_name, 's'+ str(seed_value) + 'k'+str(index+1) +'_test_' +  'x.csv') 
                val_x = pd.read_csv(path_r_set_x , header = 0) #rows: features, colulms: samples 
                #val_x = val_x.set_index('Unnamed: 0')
                #val_x = val_x.set_index()
                val_x = val_x.values
                val_x = np.transpose(val_x) #rows: samples, colulms: features
                val_x = np.stack(val_x)  
                

                path_r_set_y = os.path.join(options.original_data_folder,  options.data_name, 's'+ str(seed_value) + 'k'+str(index+1) +'_test_' +  'y.csv') 
                read_labels = pd.read_csv(path_r_set_y)
                val_y = read_labels.iloc[:,1] 
        ...
        #     bins_breaks = vis_data.build_breaks (data=data, type_bin=options.type_bin, num_bin=options.num_bin, max_v= max_v_temp, min_v = min_v_temp)
                    ...
                    for i in range(0, len(train_x)):   
                        temp_train_x.append( [vis_data.convert_bin(value=y, min_v = min_v_temp,  bins_break = bins_breaks) for y in train_x[i] ])
                   
                    train_x = np.stack(temp_train_x)
                    for i in range(0, len(val_x)):   
                        temp_val_x.append( [vis_data.convert_bin(value=y, min_v = min_v_temp,  bins_break = bins_breaks) for y in val_x[i] ])
                    ...
                    if options.test_ext == 'y':     
                        for i in range(0, len(v_data)):   
                            temp_v_data.append( [vis_data.convert_bin(value=y,  min_v = min_v_temp,  bins_break = bins_breaks) for y in v_data[i] ])
                ...
                # modified by tbtoan  ## BEGIN
                # if options.type_emb != 'none': # added by tbtoan
                if options.type_emb != 'none' and options.type_emb != 'qrcode': # added by nthai, 27/09/2023
                    title_cols = np.array([["bins for classification"]])           
                    np.savetxt(f,title_cols, fmt="%s",delimiter="/")               
                    #save information on histogram of bins
                    w_interval = float(max_v_temp - min_v_temp) / options.num_bin
            ...
            if options.type_run  in ['vis','visual']:  
                ...
                if options.num_classes >= 2:
                    if options.model in ['svm_model']:
                        ...
                        try:
                            train_y=kr.utils.to_categorical(train_y,  num_classes = options.num_classes )
                            val_y = kr.utils.to_categorical(val_y ,  num_classes = options.num_classes )
                            if options.test_ext == 'y':
                                v_labels = kr.utils.to_categorical(v_labels ,  num_classes = options.num_classes )
                        except Exception as exception:
                            train_y= tensorflow.keras.utils.to_categorical(train_y,  num_classes = options.num_classes )
                            val_y = tensorflow.keras.utils.to_categorical(val_y ,  num_classes = options.num_classes )
                            if options.test_ext == 'y':
                                v_labels = tensorflow.keras.utils.to_categorical(v_labels ,  num_classes = options.num_classes )
                ...
                if options.model in ['svm_model','dtc_model', 'rf_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    ...
                    # 27/09/2023, add model structure to log
                    # from contextlib import redirect_stdout
                    # with open(prefix_file_log+"_sum.txt", 'a') as f:
                    ...
                    #     with redirect_stdout(f):
                    #         model.get_params()
                    # print('model.get_params()model.get_params()model.get_params()model.get_params()')
                    #str_model = model.get_params()
                    #print(str_model)

                    f = open(prefix_file_log+"_sum.txt",'a')
                    np.savetxt(f,(model,model.get_params()), delimiter=" ", fmt="%s")
                    f.close()
            ...
            if options.type_run in ['vis','visual']: #only visual, not learning
                ...
                if options.model in ['svm_model', 'rf_model','dtc_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    ...
                    if options.num_classes >= 2:
                        ...
                        try:
                            train_auc_score = roc_auc_score(train_y, Y_pred, average='micro')
                        except:
                            Y_pred = model._predict_proba_lr(train_x)
                            train_auc_score = roc_auc_score(train_y, Y_pred, multi_class='ovr')
                        try:
                            train_acc.append ( accuracy_score(train_y, Y_pred))
                        except:
                            train_acc.append ( accuracy_score(train_y, Y_pred.argmax(axis=1)))

                        train_loss.append ( log_loss(train_y, Y_pred[:,]))  
                        
                        if options.model in ['rf_model','svm_model']:
                            Y_pred_mt = model.predict(train_x)
                            print("predicted")
                            print(Y_pred_mt)
                            print("standard")
                            print(train_y)
                            train_acc.append(accuracy_score(train_y, Y_pred_mt))
                            Y_pred_val_mt = model.predict(val_x)                        
                            val_acc.append ( accuracy_score(val_y, Y_pred_val_mt))
                        else:
                        ...
                        try:
                            ...
                            train_acc.append (accuracy_score(train_y, Y_pred.round()))
                            train_loss.append (log_loss(train_y, Y_pred))
                        
                            Y_pred_val = model.predict(val_x)
                            #val_acc.append ( accuracy_score(val_y, Y_pred_val[:,1].round()))
                            val_acc.append ( accuracy_score(val_y, Y_pred_val.round()))
                            #val_loss.append ( -1)
                            val_loss.append ( log_loss(val_y, Y_pred_val))
                    ...
                    # if options.num_classes > 2:
                        # train_mmc = -2
                        #train_mmc = matthews_corrcoef(train_y,Y_pred.round()) # ValueError: multilabel-indicator is not supported
                    try:
                        train_mmc = matthews_corrcoef(train_y.argmax(axis=1), Y_pred.argmax(axis=1))
                    except:
                        #print('train_y')
                        #print(train_y.argmax(axis=1))
                        #print(Y_pred.argmax(axis=1))
                        train_mmc = matthews_corrcoef(train_y, Y_pred.argmax(axis=1)) 
                    # else:
                        # train_mmc = matthews_corrcoef(train_y,Y_pred.round())
                ...
                #train
                if options.model not in ['svm_model', 'rf_model','dtc_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    ...
                    ep_stopped = 0
                    early_stop_1se.append(0)

                    train_acc_1se.append (0)
                    train_acc_1se_begin.append (0)
                    train_loss_1se.append (0)
                    train_loss_1se_begin.append (0)
                ...
                if options.num_classes >= 2:   
                    ...
                    #        cm = multilabel_confusion_matrix(train_y,   Y_pred_mt )
                    #    else:
                    ...
                    if options.num_classes > 2: 
                        ...
                        # print(y_pred)
                        try:
                            cm = confusion_matrix(train_y.argmax(axis=1), y_pred.argmax(axis=1))
                        except:
                            cm = confusion_matrix(train_y, y_pred)
                ...
                if options.model in ['svm_model', 'rf_model','dtc_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    ...
                    try:
                        ...
                        Y_pred = model._predict_proba_lr(val_x)
                    if options.debug == 'y':
                    ...
                    if options.num_classes >= 2:
                        #val_auc_score = -1
                        #Y_pred_sc = model.predict(val_x)#, verbose=2)
                        Y_pred_val_mt = model.predict(val_x)      
                        try:
                            val_auc_score = roc_auc_score(val_y, Y_pred_val_mt, average='micro')
                        except:
                            val_auc_score = roc_auc_score(val_y, Y_pred, multi_class='ovr')
                        ...
                        try:
                            val_mmc = matthews_corrcoef(val_y,Y_pred_val_mt[:,1].round())
                        except:
                            print('Y_pred[:,1]')
                            print(val_y)
                            print(Y_pred_val_mt)
                            # train_y.argmax(axis=1), y_pred.argmax(axis=1))
                            try: # for rf
                                val_mmc = matthews_corrcoef(val_y.argmax(axis=1),Y_pred_val_mt.argmax(axis=1))
                            except: # for svm
                                val_mmc = matthews_corrcoef(val_y,Y_pred_val_mt)
                                #val_mmc = matthews_corrcoef(val_y,Y_pred_val_mt)
                    else:
                    ...
                    if options.num_classes > 2:
                        #val_auc_score = -1
                        val_auc_score = roc_auc_score(val_y, Y_pred, average='micro')
                    ...
                    try:
                        val_mmc = matthews_corrcoef(val_y.argmax(axis=1), Y_pred.argmax(axis=1))
                    except:
                        val_mmc = matthews_corrcoef(val_y, Y_pred.argmax(axis=1))
                ##print(Y_pred)
                ...
                if options.num_classes>=2:     
                    ...
                    #print(cm.ravel())
                    #Y_pred_multi_val = model.predict(val_x)
                    #cm = multilabel_confusion_matrix(val_y,Y_pred_multi_val)
                    #tn, fp, fn, tp = cm.ravel()
                    #if options.num_classes > 2: 
                    tn = -1
                    fp = -1
                ...
                if options.num_classes >= 2:
                    ...
                    if options.model not in ['svm_model', 'rf_model','dtc_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                        #print(val_y)
                        #print(Y_pred.round())
                        y_pred_sc_dl = model.predict(val_x)
                        val_y_classes = y_pred_sc_dl[:,].round()
                        print(val_y_classes)
                        val_pre = precision_score(val_y,val_y_classes, average='macro')
                        val_recall = recall_score(val_y,val_y_classes, average='macro')
                        val_f1score = f1_scores_computation (val_y,val_y_classes, average='macro')
                        print('val_f1score=========')
                        print(val_f1score)
                    else:
                        #print(Y_pred_val_mt)
                        val_pre = precision_score(val_y,Y_pred_val_mt, average='macro')
                        val_recall = recall_score(val_y,Y_pred_val_mt, average='macro')
                        val_f1score = f1_scores_computation (val_y,Y_pred_val_mt, average='macro')
                        print('val_f1score=========')
                        print(val_f1score)
...
def deepmg_visual(options,args):
    ...
    prefix_file_log = res_dir + "/" +  "hdjdjdfile"
            ...
            if options.type_run  in ['vis','visual']:  
                ...
                if special_usecase in ['predict']:   
                    ...
                    if len( labels) > 0 :#if existing the label
                        ...
                        np.savetxt(f,title_cols, fmt="%s",delimiter="\t") 
                        print(Y_pred_v_proba)
                        np.savetxt(f,np.c_[Y_pred_v_proba,Y_pred_v_proba.round(),val_y], fmt="%s",delimiter="\t")
                        f.close() 
                    else:
                        f=open(prefix_file_log+"_predicted.txt",'w')  #create new
